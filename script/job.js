...
function renderReject() {
    ...
    <h1 class="company-name text-xl font-bold main-color mb-1">${reject.jobName}</h1>
        ...
        <p class="position gray-color">${reject.jobDesignation}</p>
    </div>
<p class="gray-color text-sm space-x-3"><span class="location">${reject.jobPlace} </span>• <span class="job-time">${reject.jobTime} </span> <span class="salary">${reject.jobMoney}</span></p>
    <!-- badge -->
    <div class="badge badge-md bg-red-100 main-color p-4 font-bold">${reject.jobBadge.innerText}</div>
    <p class="description">${reject.jobDescription}</p>
